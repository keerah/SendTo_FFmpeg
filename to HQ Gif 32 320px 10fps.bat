...
SET /A wset.fps=10
SET "wset.hline1=Preset: Gif 32 colors, 320px width, %wset.fps% fps, 2 pass"
SET "wset.params=-v %vbl% -hide_banner -stats -thread_queue_size 256"
SET "wset.prepass=-vf "fps=%wset.fps%,scale=320:-1:flags=lanczos,palettegen=max_colors=32:stats_mode=full""
SET "wset.videocomp=-filter_complex "fps=%wset.fps%,scale=320:-1:flags=lanczos[x];[x][1:v]paletteuse""
...
IF %dscr% GTR 0 (SET "wset.dscr=_hqgif32_320_10") ELSE (SET "wset.dscr=")

...
ECHO [---  Preset: 420, veryslow, crf 18, 1080p, FILM, kf 2 sec, Audio Copy               ---]
...
IF %dscr% GTR 0 (SET "dscrName=_420_crf18_1080p") ELSE (SET "dscrName=")
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -stats -i "!argVec[%%i]!" -c:v libx264 -preset veryslow -crf 18 -pix_fmt yuv420p -vf scale=1920:1080 -sws_flags bicubic -tune film -force_key_frames 0:00:02 -c:a copy -y "!argVn[%%i]!%dscrName%.mp4"
)

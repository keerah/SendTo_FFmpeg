...
ECHO [---  Preset: 420 main 4.0, veryslow, crf 14, FILM, kf 2sec, Audio aac128      ---]
...
IF %dscr% GTR 0 (SET "dscrName=_420_crf14_aac128") ELSE (SET "dscrName=")
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -stats -i "!argVec[%%i]!" -c:v libx264 -profile:v main -level 4.0 -preset veryslow -crf 14 -pix_fmt yuv420p -tune film -force_key_frames 0:00:02 -c:a aac -b:a 128k -y "!argVn[%%i]!%dscrName%.mp4"
)

...
ECHO [---  Preset: 420 main 4.0, Slower, crf 14, tune GRAIN, kf 2sec, Audio AAC 128 ---]
IF %1.==. (
	ECHO [---------------------------------------------------------------------------------]
	...
	ECHO [---------------------------------------------------------------------------------]
	ECHO [     Transcoding...                                                              ]
	"c:\Program Files\ffmpeg\bin\ffmpeg.exe" -i %1 -c:v libx264 -profile:v main -level 4.0 -preset veryslow -crf 14 -pix_fmt yuv420p -tune grain -force_key_frames 0:00:02 -c:a aac -b:a 128k -y %~n1_420_highest.mp4
)
ECHO [---------------------------------------------------------------------------------]
ECHO [---  SERVED                                                                      ]
ECHO [---------------------------------------------------------------------------------]

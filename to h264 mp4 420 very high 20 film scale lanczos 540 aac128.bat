...
SET "wset.hline1=Preset: h264 mp4 420, veryslow, crf 24, lanczos 540p, Film, kf 2 sec, aac128"
SET "wset.params=-v %vbl% -hide_banner -stats"
SET "wset.videocomp=-c:v libx264 -preset veryslow -crf 24 -pix_fmt yuv420p -vf scale=-1:540 -sws_flags lanczos -tune film -force_key_frames 0:00:02"
	REM The output video will have keyframes each 2 seconds due to -force_key_frames 0:00:02
	REM scales vertically to fit 540px using lanczos algo
SET "wset.audiocomp=-c:a aac -b:a 128k"
IF %quietover% == 1 (SET "wset.over=-y") ELSE (SET "wset.over=")
IF %dscr% GTR 0 (SET "wset.dscr=_420_crf24_x540_aac128") ELSE (SET "wset.dscr=")

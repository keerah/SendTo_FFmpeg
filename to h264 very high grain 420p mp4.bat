...
IF %argCount% GTR 1 (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -i "!argVec[%%i]!" -c:v libx264 -profile:v main -level 4.0 -preset veryslow -crf 18 -pix_fmt yuv420p -tune grain -force_key_frames 0:00:02 -c:a copy -y "!argVn[%%i]!_420_veryhigh.mp4"
	)

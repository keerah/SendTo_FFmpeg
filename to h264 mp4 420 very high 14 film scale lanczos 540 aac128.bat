...
ECHO [---  Preset: 420, veryslow, crf 14, scale lanczos 540p, FILM, kf 2 sec, Audio aac128---]
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -stats -i "!argVec[%%i]!" -c:v libx264 -preset veryslow -crf 14 -pix_fmt yuv420p -vf scale=-1:540 -sws_flags lanczos -tune film -force_key_frames 0:00:02 -c:a aac -b:a 128k -y "!argVn[%%i]!%dscrName%.mp4"
)

...
SET "wset.hline1=Preset: PNG, compression 9, resize to width 400"
SET "wset.params=-v %vbl% -hide_banner -stats"
SET "wset.videocomp=-vf scale=400:-1 -sws_flags lanczos -compression_level 9"
SET "wset.audiocomp="
IF %quietover% == 1 (SET "wset.over=-y") ELSE (SET "wset.over=")
IF %dscr% GTR 0 (SET "wset.dscr=_png400low") ELSE (SET "wset.dscr=")

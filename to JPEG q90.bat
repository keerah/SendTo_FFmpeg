...
IF %dscr% GTR 0 (SET "dscrName=_jpeg90") ELSE (SET "dscrName=") 
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -i "!argVec[%%i]!" -vf "scale=in_range=mpeg:out_range=full" -q:v 1 -y "!argVn[%%i]!%dscrName%_%%04d.jpg"
)

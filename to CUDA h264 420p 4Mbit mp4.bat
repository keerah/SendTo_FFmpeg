...
ECHO [---  Preset: CUDA 420, 4 Mbps, keyfr 2 sec, Audio Copy                        ---]
...
IF %argCount% GTR 1 (
	...
	IF %dscr% GTR 0 (SET "dscrName=_cuda420_4Mbit") ELSE (SET "dscrName=")
	...
	FOR /L %%i IN (1,1,%argCount%) DO (
		...
		"%ffpath%ffmpeg.exe" -v %vbl% -vsync 0 -hwaccel cuvid -i "!argVec[%%i]!" -c:v h264_nvenc -preset slow -b:v 4M -pix_fmt yuv420p -force_key_frames 0:00:02 -c:a copy -y "!argVn[%%i]!%dscrName%.mp4"
	)
...
rem Just one hint from me on -b:v 4M argument, which defines the bitrate for the output.

...
ECHO [---  Preset: CUDA 420 main, slow, 20 Mbps, kf 2 sec, Audio Copy               ---]
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -stats -vsync 0 -hwaccel cuvid -i "!argVec[%%i]!" -c:v h264_nvenc -profile:v main -preset slow -b:v 20M -pix_fmt yuv420p -force_key_frames 0:00:02 -c:a copy -y "!argVn[%%i]!%dscrName%.mp4"
)

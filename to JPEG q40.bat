...
SET "wset.hline1=Preset: JPEG quality 40"
SET "wset.params=-v %vbl% -hide_banner -stats"
SET "wset.videocomp=-vf "scale=in_range=mpeg:out_range=full" -q:v 6"
SET "wset.audiocomp="
IF %quietover% == 1 (SET "wset.over=-y") ELSE (SET "wset.over=")
IF %dscr% GTR 0 (SET "wset.dscr=_jpeg40") ELSE (SET "wset.dscr=")

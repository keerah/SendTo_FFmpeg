...
IF %argCount% GTR 1 (
	...
	FOR /L %%i IN (1,1,%argCount%) DO (
		...
		"c:\Program Files\ffmpeg\bin\ffmpeg.exe" -v warning -i "!argVec[%%i]!" -vf fps=15,scale=640:-1:flags=lanczos,palettegen -y "!argVec[%%i]!"_palette.png  
		ECHO [---------------------------------------------------------------------------------]
		ECHO [     Encoding file %%i of %argCount%
		ECHO [     STAGE 2: Encoding to Gif using the generatied palette                       ]
		"c:\Program Files\ffmpeg\bin\ffmpeg.exe" -v warning -i "!argVec[%%i]!" -i "!argVec[%%i]!"_palette.png -filter_complex "fps=15,scale=640:-1:flags=lanczos[x];[x][1:v]paletteuse" -y "!argVec[%%i]!"_hqgif.gif 
...
rem All videos rescaled to 640 pixels and stripped to 15 fps. The script doesn't have alpha channel support yet though.

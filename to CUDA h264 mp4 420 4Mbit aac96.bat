...
ECHO [---  Preset: CUDA 420 main, slow, 4 Mbps, no kf, Audio aac96                  ---]
...
IF %dscr% GTR 0 (SET "dscrName=_cuda420_5Mbit_aac96") ELSE (SET "dscrName=")
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -stats -vsync 0 -hwaccel cuvid -i "!argVec[%%i]!" -c:v h264_nvenc -profile:v main -preset slow -b:v 4M -pix_fmt yuv420p -c:a aac -b:a 96k -y "!argVn[%%i]!%dscrName%.mp4"
)

...
ECHO [---  Preset: CUDA 420 main, 4 Mbps, nokeyfr, Audio AAC 96                     ---]
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -vsync 0 -hwaccel cuvid -i "!argVec[%%i]!" -c:v h264_nvenc -profile:v main -preset slow -b:v 4M -pix_fmt yuv420p -c:a aac -b:a 96k -y "!argVn[%%i]!%dscrName%.mp4"
)

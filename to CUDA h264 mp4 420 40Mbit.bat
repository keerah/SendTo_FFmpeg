...
ECHO [---  Preset: CUDA 420 main 4.0, slow, 40 Mbps, kf 2 sec, Audio Copy           ---]
...
IF %dscr% GTR 0 (SET "dscrName=_cuda420_40Mbit") ELSE (SET "dscrName=")
...
FOR /L %%i IN (1,1,%argCount%) DO (
	...
	"%ffpath%ffmpeg.exe" -v %vbl% -hide_banner -stats -vsync 0 -hwaccel cuvid -i "!argVec[%%i]!" -c:v h264_nvenc -profile:v main -preset slow -b:v 40M -pix_fmt yuv420p -force_key_frames 0:00:02 -c:a copy -y "!argVn[%%i]!%dscrName%.mp4"
)
